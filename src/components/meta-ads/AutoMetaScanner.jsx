...
import { Search, Play, X, Loader2, ExternalLink, CheckCircle2, AlertTriangle, Upload } from 'lucide-react'
...
export default function AutoMetaScanner({ onPrefill }) {
  ...
  return (
    <div style={{ marginBottom: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        {open && (
          <button
            ...
            style={{ padding: '0.5rem 0.75rem', border: '1px solid var(--color-border)', borderRadius: '10px', background: 'white', fontWeight: '600' }}
          >
            Temizle
          </button>
        )}
      ...
      {open && (
        <div className="glass-panel" style={{ marginTop: '0.75rem', padding: '1rem', borderRadius: '16px' }}>
          {/* Form */}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '0.75rem' }}>
            {items.map(ad => (
              <div key={ad.id} className="glass-panel" style={{ padding: '0.75rem', borderRadius: '14px', border: '1px solid var(--color-border)', position: 'relative' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                  <div>
                    <div style={{ fontWeight: '700' }}>{ad.page_name || 'Bilinmeyen Sayfa'}</div>
